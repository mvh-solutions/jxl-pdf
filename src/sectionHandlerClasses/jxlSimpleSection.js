...
class jxlSimpleSection extends Section {
  ...
  async doSection({ section, templates, manifest, options }) {
    const jsonFile = fse.readJsonSync(
    ...
    if (section.content.glossNotes) {
      const pivotRows = fse
        .readFileSync(
          resolvePath(
            path.join(
              section.content.glossNotes[0].pivot,
              `${section.bcvRange}.tsv`,
            ),
          ),
        )
        .toString()
      ...
      const notesRows = fse
        .readFileSync(
          resolvePath(
            path.join(
              section.content.glossNotes[0].notes,
              `${section.bcvRange}.tsv`,
            ),
          ),
        )
    ...
    if (section.content.topTextSrc) {
      extraTexts["top"] = {
        id: `xxx_top`,
        path: resolvePath(section.content.topTextSrc),
        type: "greek",
        text: "top text",
      };
    }
    if (section.content.bottomTextSrc) {
      extraTexts["bottom"] = {
        id: `xxx_bottom`,
        path: resolvePath(section.content.bottomTextSrc),
        type: "translation",
        text: "bottom text",
      };
    }
    ...
    if (docSpecs.length > 0) {
      pk = pkWithDocs(section.bcvRange, docSpecs, options.verbose);
    }
