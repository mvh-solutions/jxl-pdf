const quoteForCv = (pk, sentenceRecord, bookCode, cv) => {
  ...
  // console.log("errors", cvResult.errors);
...
const getCVTexts = (bookCode, pk) => {
  const cvQuery = `{
    ...
    `;
  const result = pk.gqlQuerySync(cvQuery).data.docSets.map((ds) => ({
    id: ds.id,
    chapters: ds.document.cvIndexes.map((cvi) => ({
      chapterN: cvi.chapter,
      verseRanges: cvi.verseNumbers.filter((vr) => vr.range.includes("-")),
      verses: cvi.verses
  ...
  for (const ds of result) {
    ...
    for (const chapter of ds.chapters) {
      ...
      for (const vr of chapter.verseRanges) {
        verseRanges[chapter.chapterN][vr.number] = !seenRanges.has(vr.range)
          ? vr.range
          : null;
  ...
  for (const chapter of result[0].chapters) {
    for (const verse of chapter.verses) {
      ...
      if (
        verseRanges[chapter.chapterN] &&
        verse.verseN in verseRanges[chapter.chapterN]
      ) {
        // verseRange record exists
        ...
        for (const ds of Object.keys(cvLookup)) {
          if (cvLookup[ds][chapter.chapterN]) {
            ...
            } else if (verse.verseN in cvLookup[ds][chapter.chapterN]) {
              // verseRange for which there is single verse content;
              if (!verseRanges[chapter.chapterN][verse.verseN]) {
                // verseRange is null, so already handled for a previous verse
                continue;
              }
              for (const vrNumber of vrNumbers(
                verseRanges[chapter.chapterN][verse.verseN],
              )) {
                if (!retRecord.texts[ds]) {
                  retRecord.texts[ds] =
                    cvLookup[ds][chapter.chapterN][vrNumber] || "??";
                } else {
                  retRecord.texts[ds] +=
                    ` ${cvLookup[ds][chapter.chapterN][vrNumber] || "??"}`;
                }
        ...
        for (const ds of Object.keys(cvLookup)) {
          if (
            cvLookup[ds][chapter.chapterN] &&
            verse.verseN in cvLookup[ds][chapter.chapterN]
          ) {
